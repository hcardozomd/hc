...
class builtin_enes_Collins {
    ...
    async displayName() {
        ...
        if (locale.indexOf('ES') != -1) return 'collinsEngEsp)';
        if (locale.indexOf('TW') != -1) return 'collinsEngEsp)';
    ...
    async findCollins(word) {
        ...
        for (const def of defs) {
            let definition = '';
            let pos = def.pos_en;
            let esp_tran = def.def_es;
            let eng_tran = def.def_en;
            pos = pos ? `<span class="pos">${pos}</span>` : '';
            esp_tran = esp_tran ? `<span class="esp_tran">${esp_tran}</span>` : '';
            eng_tran = eng_tran ? `<span class="eng_tran">${eng_tran.replace(RegExp(expression, 'gi'),`<b>${expression}</b>`)}</span>` : '';
            definition = `${pos}<span class="tran">${eng_tran}${esp_tran}</span>`;
            ...
            if (def.ext && def.ext.length > 0 && maxexample > 0) {
                ...
                for (const [idx, ex] of def.ext.entries()) {
                    ...
                    let esp_sent = ex.ext_es;
                    let eng_sent = ex.ext_en.replace(RegExp(expression, 'gi'),`<b>${expression}</b>`);
                    definition += `<li class='sent'><span class='eng_sent'>${eng_sent}</span><span class='esp_sent'>${esp_sent}</span></li>`;
                }
                definition += '</ul>';
            }
    ...
    renderCSS() {
        let css = `
            <style>
                ...
                span.esp_tran {color:#0d47a1;}
                ul.sents {font-size:0.8em; list-style:square inside; margin:3px 0;padding:5px;background:rgba(13,71,161,0.1); border-radius:5px;}
                li.sent  {margin:0; padding:0;}
                span.eng_sent {margin-right:5px;}
                span.esp_sent {color:#0d47a1;}
            </style>`;
        return css;
    }
}